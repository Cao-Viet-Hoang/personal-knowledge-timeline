...
2. **Prompts are English, output language is user-selected** — system prompts and user-turn prompts in `ai-actions.js` MUST always be written in English so the canonical instructions stay diffable and consistent. Each entry-level AI action (`enrichEntry`, `generateTags`, `generateTitle`, `generateSummary`, `expandContent`, `parseUrl`) takes a `{ language: "en" | "vi" }` option and instructs the model to reply in that language. The entry form exposes an EN/VI toggle (stored on the entry as `aiLanguage`, default `"en"`) that drives this option — do NOT sniff the source text's language, always respect the explicit toggle. The only action that takes a direct language argument is `translateText(text, targetLang)`, which is explicitly for on-demand translation. In edit mode, Translate → English/Vietnamese also translates the AI summary alongside content/excerpt and flips the `aiLanguage` toggle to match the new language.
